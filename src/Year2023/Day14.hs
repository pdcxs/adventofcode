...
tilt width height rows cols dir ((x, y) : ps) =
 ...
 where
  ...
  bound
    | fst dir < 0 || snd dir < 0 = 0
    | fst dir > 0 = width - 1
    | otherwise = height - 1
  (rcd', p) = update dir rcd bound key val
  p' =
    if fst dir == 0 then (x, p) else (p, y)
...
solution1 s = print (getAnswer height ps')
 where
  ...
  ps' =
    tilt width height rows cols (0, -1) sps
...
solution2 s = print (getAnswer height $ ps' !! idx)
 where
  ...
  preLen =
    fromJust $ elemIndex (head cycles) ps'
  idx =
    preLen
      + ((1000000000 - preLen - 1) `mod` n)
...
cycleTilt width height rows cols ps = ps' : cont
 where
  ps1 =
    tilt width height rows cols (0, -1) ps
  sps1 = sortBy (comparing fst) ps1
  ps2 =
    tilt width height rows cols (-1, 0) sps1
  sps2 = sortBy (comparing (Down . snd)) ps2
  ps3 =
    tilt width height rows cols (0, 1) sps2
  sps3 = sortBy (comparing (Down . fst)) ps3
  ps4 =
    tilt width height rows cols (1, 0) sps3
