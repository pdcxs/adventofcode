...
type Candidate =
  M.Map Score [(State, [State])]
type Score = Int

-- state, score, prev states
type Record =
  M.Map State (Score, [State])

processInput ::
  String -> (Pos, Pos, S.Set Pos)
...
search ps candidate rcd
 ...
 where
  ...
  candidate'' =
    M.unionWith
      (++)
      (nextStates ps score st rcd)
      candidate'
  rcd' = updateRecord score st prv rcd

getNext ::
  Candidate ->
  ((Score, (State, [State])), Candidate)
...
updateRecord score st prev rcd =
  case M.lookup st rcd of
    ...
    Just (s, prevs) ->
      if score <= s
        then
          M.insert st (score, prev ++ prevs) rcd
        else rcd
...
getRecord s =
  ( end
  , search ps initialCandidate M.empty
  )
 where
...
solution1 :: String -> IO ()
solution1 =
  print . uncurry findMinScore . getRecord

solution2 :: String -> IO ()
solution2 =
  print
    . uncurry findPositionCount
    . getRecord
