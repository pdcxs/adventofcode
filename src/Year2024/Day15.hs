...
printMap ::
  Box -> Wall -> Pos -> Char -> String
printMap boxes walls pos c =
  unlines $
    map (prefix . map locChar) locs
 where
  ...
  locChar p
    | p == pos = c
    | any
        (\b -> head b == p && length b == 1)
        boxes =
        'O'
    | any
        (\b -> head b == p && length b == 2)
        boxes =
        '['
    | any
        (\b -> last b == p && length b == 2)
        boxes =
        ']'
    | p `elem` walls = '#'
    | otherwise = '.'
