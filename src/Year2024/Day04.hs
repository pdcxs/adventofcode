...
getSize grid =
  (V.length (V.head grid), V.length grid)

posStrCount :: Grid -> Pos -> Int
posStrCount grid (r, c) =
  length $ filter isXMAS [hs, vs, ds, ds']
 where
  n = [0 .. 3]
  h = map (\i -> (r, c + i)) n
  v = map (\i -> (r + i, c)) n
  ...
  d' = map (\i -> (r + i, c - i)) n
  hs =
    map (get grid) $ filter (inGrid grid) h
  vs =
    map (get grid) $ filter (inGrid grid) v
  ds =
    map (get grid) $ filter (inGrid grid) d
  ds' =
    map (get grid) $ filter (inGrid grid) d'
