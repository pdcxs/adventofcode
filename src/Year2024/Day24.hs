...
getFullAdder n cin f =
 ...
 where
  d =
    if n >= 10 then show n else '0' : show n
  xn = 'x' : d
  yn = 'y' : d
  zn = 'z' : d

  a =
    fst . head $ filter (matchXor xn yn) f
  xorToCin =
    if n > 0
      then
        map (extract cin . snd) $
  ...
  aerr2 =
    [ a | n > 0, not (any (matchAnd a cin) f)
    ]
  ...
  b =
    fst . head $ filter (matchAnd xn yn) f
  ...
  (c, cerr')
    ...
    | otherwise =
        ( fst . head $ filter (containsAnd cin) f
        , [b]
        )
  ...
  cout
    ...
    | otherwise =
        fst . head $ filter (containsOr a) f
  couterr = [cout | cout == zn]
  cout' = if cout == zn then z else cout

containsOr ::
  String -> (String, Expr) -> Bool
...
solution2 s =
 ...
 where
  ...
  n =
    length (head $ splitWhen null $ lines s)
      `div` 2
  go i cin =
    if i < n
      then
